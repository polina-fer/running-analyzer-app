...
def draw_overlays(frame, last_landmarks_data, avg_cadence, right_step_count, left_step_count,
                  current_status_event, latest_ai_results, leg_is_on_ground, fw, fh):
    """Draw annotations on any frame using the last known landmark positions."""
    if last_landmarks_data is None:
        return frame

    display = frame.copy()
    overlay_layer = frame.copy()

    r_hip, r_knee, r_ankle = last_landmarks_data['r_hip'], last_landmarks_data['r_knee'], last_landmarks_data['r_ankle']
    l_hip, l_knee, l_ankle = last_landmarks_data['l_hip'], last_landmarks_data['l_knee'], last_landmarks_data['l_ankle']
    r_shld, r_elb, r_wrst = last_landmarks_data['r_shld'], last_landmarks_data['r_elb'], last_landmarks_data['r_wrst']
    l_shld, l_elb, l_wrst = last_landmarks_data['l_shld'], last_landmarks_data['l_elb'], last_landmarks_data['l_wrst']
    r_knee_ang = last_landmarks_data['r_knee_ang']
    l_knee_ang = last_landmarks_data['l_knee_ang']
    r_elb_ang = last_landmarks_data['r_elb_ang']

    torso_pts = np.array([r_shld, l_shld, l_hip, r_hip], np.int32)
    cv2.fillPoly(overlay_layer, [torso_pts], (0, 255, 0))
    cv2.addWeighted(overlay_layer, 0.15, display, 0.85, 0, display)
    cv2.polylines(display, [torso_pts], True, (255, 255, 255), 2)
    cv2.line(display, tuple(r_shld), tuple(l_hip), (255, 255, 255), 1)
    cv2.line(display, tuple(l_shld), tuple(r_hip), (255, 255, 255), 1)
    cv2.line(display, tuple(r_ankle), tuple(l_ankle), (255, 0, 255), 2)

    for h, k, a, c in [(r_hip, r_knee, r_ankle, (0, 255, 0)), (l_hip, l_knee, l_ankle, (0, 255, 255))]:
        cv2.line(display, tuple(h), tuple(k), c, 3)
        cv2.line(display, tuple(k), tuple(a), c, 3)
    for s, e, w in [(r_shld, r_elb, r_wrst), (l_shld, l_elb, l_wrst)]:
        cv2.line(display, tuple(s), tuple(e), (255, 165, 0), 3)
...
def analyze_video_stream(video_path, output_path=None):
        ...
        while video_capture.isOpened():
            ...
            if results.pose_landmarks:
                ...
                last_landmarks_data = {
                    ...
                    'r_shld': r_shld, 'r_elb': r_elb, 'r_wrst': r_wrst,
                    'l_shld': l_shld, 'l_elb': l_elb, 'l_wrst': l_wrst,
                    'r_knee_ang': r_knee_ang, 'l_knee_ang': l_knee_ang, 'r_elb_ang': r_elb_ang,
                }
            ...
            if processed_count % 5 == 0:
                pct = min(int((frame_count / total_frames) * 100), 99) if total_frames > 0 else 0
                preview_w = 480
                preview_h = int(display_frame.shape[0] * preview_w / display_frame.shape[1])
                preview = cv2.resize(display_frame, (preview_w, preview_h))
                _, jpeg = cv2.imencode('.jpg', preview, [cv2.IMWRITE_JPEG_QUALITY, 60])
                frame_b64 = base64.b64encode(jpeg.tobytes()).decode('utf-8')
                yield {
                    "type": "progress",
                    "pct": pct,
                    "steps": right_step_count + left_step_count,
                    "cadence": round(avg_cadence, 1),
                    "scores": {k: int(v) for k, v in latest_ai_results.items()},
                    "frame": frame_b64,
                }
    ...
    if not done_steps:
        yield {"type": "error", "message": "No steps detected. Make sure the video shows a clear side view of someone running.", "total_steps": 0}
        return
...
def analyze_video(video_path, output_path=None):
    for event in analyze_video_stream(video_path, output_path):
        if event["type"] == "done":
            return {k: v for k, v in event.items() if k != "type"}
        if event["type"] == "error":
            return {"error": event["message"], "total_steps": event.get("total_steps", 0)}
